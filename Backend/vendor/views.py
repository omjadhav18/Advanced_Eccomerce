...
from store.serializers import ProductSerializer,CategorySerializer,CartSerializer,CartOrderSerializer,CouponSerializer,NotificationSerializer,ReviewSerializer,WishlistSerializer,SummarySerializer,CartOrderItemSerializer,EaningSerializer,CouponSummarySerializer,NotificationSummarySerializer,VendorSerializer,SpecificationSerializer,ColorSerializer,SizeSerializer,GallerySerializer
...
class DashboardStatsAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        revenue = CartOrderItem.objects.filter(vendor=vendor).aggregate(
        total_revenue=models.Sum(models.ExpressionWrapper(models.F('sub_total') + models.F('shipping_amount'), output_field=models.DecimalField()))
        )['total_revenue'] or 0
...
class RevenueAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return CartOrderItem.objects.filter(vendor=vendor).aggregate(
        total_revenue=models.Sum(models.ExpressionWrapper(models.F('sub_total') + models.F('shipping_amount'), output_field=models.DecimalField()))
        )['total_revenue'] or 0
...
class EarningAPIView(generics.ListAPIView):
    serializer_class = EaningSerializer
    permission_classes = [AllowAny]
    ...
    def get_queryset(self):
        ...
        monthly_revenue = CartOrderItem.objects.filter(vendor=vendor,date__gte=one_month_ago).aggregate(
        total_revenue=models.Sum(models.ExpressionWrapper(models.F('sub_total') + models.F('shipping_amount'), output_field=models.DecimalField()))
        )['total_revenue'] or 0
        total_revenue = CartOrderItem.objects.filter(vendor=vendor).aggregate(
        total_revenue=models.Sum(models.ExpressionWrapper(models.F('sub_total') + models.F('shipping_amount'), output_field=models.DecimalField()))
        )['total_revenue'] or 0
...
class CouponListCreateAPIView(generics.ListCreateAPIView):
    ...
    def create(self,request,*args,**kwargs):
        ...
        return Response({"message":"Coupan Created Successfully"},status=status.HTTP_201_CREATED)
...
class CouponStatsAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        active_coupons = Coupon.objects.filter(active=True,vendor=vendor).count()
